...
! ===============================================
      ...
      rfile='../extract_water/1deg_10m/'//trim(cname)//'.bil'
      open(11,file=rfile,form='unformatted',access='direct',recl=1*nx*ny)
      read(11,rec=1) water
      close(11)

      rfile='../add_stream/1deg_10m/'//trim(cname)//'.bil'
      open(11,file=rfile,form='unformatted',access='direct',recl=1*nx*ny)
      read(11,rec=1) stream
      close(11)

      rfile='../add_canal/1deg_10m/'//trim(cname)//'.bil'
      open(11,file=rfile,form='unformatted',access='direct',recl=1*nx*ny)
      read(11,rec=1) canal
      close(11)

      rfile='../add_river/1deg_10m/'//trim(cname)//'.bil'
      open(11,file=rfile,form='unformatted',access='direct',recl=1*nx*ny)
      read(11,rec=1) river
      close(11)
